...
def error_mail(message):
	username = 'badal@10i.co.in'
	pwd = 'Akash4051?'
	msg = MIMEMultipart()
	msg['From'] = 'badal@10i.co.in'
	...
	text = msg.as_string()
	server.sendmail('badal@10i.co.in', ['badal@10i.co.in'], text)
	server.quit()
	sys.exit()
...
def xls_mail(file_name):
	...
	server.login(username,pwd)
	text = msg.as_string()
	server.sendmail('badal@10i.co.in', ['badal@10i.co.in','kishan@10i.co.in'], text)
	server.quit()
